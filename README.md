...
可在無 API Key、無網路情況下執行核心行程規劃；CSV 住宿、景點與評論均為**示範資料**，金額為新台幣估算，沒有訂房與即時房價能力。目的地目前支援台北、東京、京都、首爾。
...
複製 `.env.example` 為 `.env`，可填 `OPENAI_API_KEY`；不填時仍可執行。勾選即時資料才呼叫外部天氣與匯率 API。日期若超出預報範圍會顯示不可用，不會捏造天氣。LLM 失敗會顯示規則式備援。公開部署的上傳知識庫存在共享記憶體中、重啟消失且不隔離使用者；**勿上傳敏感筆記**，若需多人或私人資料，先加入登入、權限與獨立持久化索引。
